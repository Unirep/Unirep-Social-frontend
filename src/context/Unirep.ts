...
import { SERVER, UNIREP_SOCIAL_ABI, UNIREP_ABI, DEFAULT_ETH_PROVIDER } from '../config'
...
export class UnirepConfig {
  ...
  async load() {
    const { unirepAddress, unirepSocialAddress } = await fetch(`${SERVER}/api/config`).then(r => r.json())
    this.unirepAddress = unirepAddress
    this.unirepSocialAddress = unirepSocialAddress
    // now load the contract specifics
    this.unirep = new ethers.Contract(unirepAddress, UNIREP_ABI, DEFAULT_ETH_PROVIDER)
    this.unirepSocial = new ethers.Contract(unirepSocialAddress, UNIREP_SOCIAL_ABI, DEFAULT_ETH_PROVIDER)
