...
import { stringifyBigInts, unSerialiseIdentity, serialiseIdentity } from '@unirep/crypto'
...
export class UnirepState extends _UnirepState {
    toJSON = (space = 0): string => {
        ...
        for (const key of epochKeys) {
            attestationsMapToString[key] = (this as any).epochKeyToAttestationsMap[key].map((n: any) => n.toJSON())
        }
        const epochTreeLeavesToString = {} as any
        for (let index in (this as any).epochTreeLeaves) {
            epochTreeLeavesToString[index] = (this as any).epochTreeLeaves[index].map(
                (l: any) =>
  ...
  static fromJSON(data: string) {
    ...
    for(let key in _unirepState.GSTLeaves){
        parsedGSTLeaves[key] = _unirepState.GSTLeaves[key].map((n: any) => BigInt(n))
    }
...
export class UserState extends _UserState {
  toJSON = (space: any): string => {
        ...
        for (let nonce = 0; nonce < (this as any).unirepState.setting.numEpochKeyNoncePerEpoch; nonce++) {
            ...
            if(attestations !== undefined) transitionedFromAttestationsToString[epk] = attestations.map((a: any) => a.toJSON())
        }
        return {
